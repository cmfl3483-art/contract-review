...
### ⚠️ 关键：develop 比 main 超前 37 个 commit

**测试环境已有、生产环境还没有的功能**（全部在 `develop` 分支，未合并到 `main`）：

#### 功能一：合规检查（核心新功能）
- 管理员可创建/管理合规规则集合（规则集合支持多条同时生效）
- 支持 Excel 批量导入规则（下载模板 → 填写 → 上传预览 → 确认导入）
- 用户可发起合规检查：上传合同文件 → 选择规则集合 → AI 自动检查 → 返回违规项列表
- 合规检查为**异步模式**：提交后立即返回，后台 Celery worker 处理，前端轮询状态
- AI 使用 DeepSeek `deepseek-chat` 模型（V3 Pro 级别）

#### 功能二：合规规则 Excel 批量导入
- 在规则集合详情页可下载标准 Excel 模板
- 支持批量上传规则（含预览确认步骤）
- 支持字段校验（rule_type/title/requirement/severity/order）
...
- 合规检查详情页滚动修复
- 新建合规检查规则集合必填校验
- CI/CD 改进：每次部署重建 backend/frontend/celery_worker 镜像

### 生产环境当前状态

生产环境（`main` 分支）**没有**上述任何合规检查功能，数据库也没有对应的表（`compliance_check_results`、`compliance_rules`、`compliance_rule_sets`）。
